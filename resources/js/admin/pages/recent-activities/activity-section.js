...
const ActivitySection = ({ title, data, ver = '1' }) => {
                ...
                <div className="px-6" key={`${record.id}.${index}`}>
                    {
                        ver == 2 ?
                        <ListItem
                            avatar={`${record.avatar || 'http://www.gravatar.com/avatar/3b3be63a4c2a439b013787725dfce802?d=mp'}`}
                            avatarStyle="h-12 w-12"
                            title={record.title}
                            titleStyle="capitalize font-semibold"
