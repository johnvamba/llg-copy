...
class CampusPortalScope implements Scope
{
    /**
    ...
    public function apply(Builder $builder, Model $model)
    {
        ...
        if(($user = auth()->user()) && session('filterOn', false)){
            if($user->hasRole('user')){
                ...
                $builder->whereHas('organizations', fn($org)=> $org->where('organizations.id', session('org_id', 0)));
            } else if($user->hasRole('campus admin')){
                $builder->where('campuses.id', session('camp_id', 0));
